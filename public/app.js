...
function renderEvents(){
  ...
  const filtered = eventsData.filter(e=> matchesEvent(e, q, m, st))
                             .sort((a,b)=> (a.dateStart||'').localeCompare(b.dateStart||''));

  eventList.innerHTML = '';
  if (!filtered.length){
    eventList.innerHTML = '<div class="card">No events yet. Add some via <code>events.json</code> or submit using the form.</div>';
    return;
  }
  filtered.forEach(e=>{
    ...
    wrap.innerHTML = `
      ...
      <div>${e.address||''}</div>
      ${e.link? `<div><a href="${e.link}" target="_blank" rel="noopener">Official Link</a></div>`: ''}
    `;
...
// ---------- Submit (EmailJS) ----------
// Where do emails go?
// → In EmailJS Dashboard: your Template defines the recipient ("To" email).
// When this form submits, EmailJS will send an email to that configured recipient.
